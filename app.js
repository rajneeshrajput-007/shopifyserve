...
app.post('/carrier_service', async (req, res) => {
  ...
  try {
    ...
    const shippingPayload = {
      ...
      dimensionUnit: "CM",
      weight: totalWeight,
      WeightUnit: "KG", // Correct casing
      quantity: totalQuantity, // Set to total quantity
      shipTo: destination.country, // Use destination country
      shipToCity: destination.city, // Use destination city
      shipToPostal: destination.postal_code, // Use destination postal code
      productTotal: items.reduce((sum, item) => sum + item.price * item.quantity, 0) // Total value of items
    };
